...
import type { EndpointSchema, GeneratedTool, JsonSchemaProperty } from "./types.js";
...
export class ToolGenerator {
    ...
    generateZodSchema(endpoint: EndpointSchema): string {
        ...
        for (const param of endpoint.parameters) {
            ...
            fields.push(this.paramToZodField(param.name, param.schema, param.required, param.description));
        }
        ...
        if (endpoint.requestBody?.schema.properties) {
            ...
            for (const [name, prop] of Object.entries(endpoint.requestBody.schema.properties)) {
                ...
                fields.push(this.paramToZodField(name, prop, isRequired, prop.description ?? ""));
            }
        }
    ...
    private generateHandler(endpoint: EndpointSchema, _toolName: string): string {
        ...
        const hasBody = !!endpoint.requestBody && (method === "post" || method === "put" || method === "patch");
        ...
        if (hasBody) {
            ...
            if (bodyParamNames.length > 0) {
                const bodyObj = bodyParamNames
                    .map((n) => `          ${this.sanitizeFieldName(n)}: params.${this.sanitizeFieldName(n)}`)
                    .join(",\n");
