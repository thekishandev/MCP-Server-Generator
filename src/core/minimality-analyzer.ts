...
export class MinimalityAnalyzer {
    ...
    analyze(
        ...
        const endpointMetrics = this.analyzeEndpoints(specDir, selectedEndpoints);
        const schemaMetrics = this.analyzeSchemas(specDir, selectedEndpoints);
        const tokenMetrics = this.analyzeTokens(
            specDir,
            selectedEndpoints,
            endpointMetrics.totalInSpecs
        );
        const refDepth = this.analyzeRefDepth(specDir, selectedEndpoints);
    ...
    formatReport(report: MinimalityReport): string {
        ...
        lines.push("╔══════════════════════════════════════════════════════════════╗");
        lines.push("║          MINIMALITY ANALYSIS REPORT                        ║");
        lines.push("╠══════════════════════════════════════════════════════════════╣");
        lines.push(`║  Capabilities: ${report.capabilities.join(", ").padEnd(44)}║`);
        lines.push(`║  Analyzed at:  ${report.analyzedAt.substring(0, 19).padEnd(44)}║`);
        lines.push("╠══════════════════════════════════════════════════════════════╣");

        // Endpoint metrics
        lines.push("║  ENDPOINT PRUNING                                          ║");
        lines.push("╠────────────────────────────────┬─────────────┬─────────────╣");
        lines.push("║  Metric                        │ Full        │ Minimal     ║");
        lines.push("╠────────────────────────────────┼─────────────┼─────────────╣");
        lines.push(
        ...
        lines.push("╠────────────────────────────────┼─────────────┼─────────────╣");
        lines.push("║  $ref RESOLUTION DEPTH         │             │             ║");
        lines.push("╠────────────────────────────────┼─────────────┼─────────────╣");
        lines.push(
        ...
        lines.push("╠════════════════════════════════════════════════════════════╣");
        lines.push(`║  Endpoints:   ${report.reduction.endpointReduction.padEnd(45)}║`);
        lines.push(`║  Schema:      ${report.reduction.schemaReduction.padEnd(45)}║`);
        lines.push(`║  Components:  ${report.reduction.componentReduction.padEnd(45)}║`);
        lines.push(`║  Tokens:      ${report.reduction.tokenReduction.padEnd(45)}║`);
        lines.push("╠════════════════════════════════════════════════════════════╣");
        ...
        lines.push("┌──────────────────────────────┬──────────┬──────────┬──────────┐");
        lines.push("│ File                         │ Size     │ Endpts   │ Relevant │");
        lines.push("├──────────────────────────────┼──────────┼──────────┼──────────┤");
    ...
    private analyzeSchemas(
                ...
                for (const method of Object.keys(methods)) {
                    if (["get", "post", "put", "delete", "patch"].includes(method)) {
                        ...
                        if (selectedPaths.has(apiPath)) {
                            ...
                            this.collectRefs(methods[method] as Record<string, unknown>, usedRefs);
                        }
                    }
                }
            }
        ...
        const minimalSpecSize = this.estimateMinimalSize(selectedEndpoints, usedRefs);
    ...
    private analyzeTokens(
        ...
        return {
            ...
            tokenReductionPercentage:
                fullServerTokens > 0
                    ? (tokensSaved / fullServerTokens) * 100
                    : 0,
            perToolTokens,
        };
    }
        ...
        for (const file of files) {
            ...
            for (const [apiPath, methods] of Object.entries(paths)) {
                ...
                for (const [method, operation] of Object.entries(methods)) {
                    ...
                    if (tracker.maxDepth > globalMaxDepth) globalMaxDepth = tracker.maxDepth;
