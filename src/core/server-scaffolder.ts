...
export class ServerScaffolder {
  ...
  private generateEnvExample(config: ServerConfig): GeneratedFile {
    ...
    return {
      ...
      content: template({
        rcUrl: config.rcUrl ?? "http://localhost:3000",
        rcAuthToken: config.rcAuthToken ?? "",
        rcUserId: config.rcUserId ?? "",
      }),
    };
  }
...
async function main() {
  ...
  console.error(\`{{serverName}} MCP server ready ({{tools.length}} tools)\`);
}
 ...
 *
 * RC_URL is resolved lazily from process.env on the first request,
 * so no credentials or configuration are needed at startup.
 *
 * Auto-generated by rc-mcp — do not edit manually.
 */
...
export class RCClient {
  ...
  private get baseUrl(): string {
    return (process.env.RC_URL ?? "http://localhost:3000").replace(/\\/$/, "");
  }

  /** Set authentication credentials (called per-request by each tool) */
...
import { z } from "zod";
import { {{toolName}}Schema, register_{{toolName}} } from "../src/tools/{{toolName}}.js";
...
describe("{{toolName}} Zod Schema", () => {
  ...
  it("should export a register function for MCP server integration", () => {
    expect(register_{{toolName}}).toBeDefined();
