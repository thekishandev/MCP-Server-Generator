...
export class GeminiCLIIntegration {
  ...
  generateExtensionManifest(
    ...
    return {
      ...
      settings: [
        {
          name: "Workspace URL",
          description: "URL of your Rocket.Chat workspace",
          envVar: "RC_URL",
        },
        {
          name: "Username",
          description: "Your Rocket.Chat username",
          envVar: "RC_USER",
        },
        {
          name: "Password",
          description: "Your Rocket.Chat password",
          envVar: "RC_PASSWORD",
          sensitive: true,
        },
      ],
  ...
  generateGeminiMd(options: GeminiIntegrationOptions): string {
    ...
    lines.push("Use these tools to interact with the Rocket.Chat workspace.");
    lines.push("");
    ...
    lines.push("- Channel names should be passed without the `#` prefix.");
    lines.push(
  ...
  createExtension(options: GeminiIntegrationOptions): string[] {
    const extDir =
      options.extensionDir ?? resolve(options.serverDir, ".gemini-extension");
