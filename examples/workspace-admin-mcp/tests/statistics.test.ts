...
import { z } from "zod";
import { statisticsSchema } from "../src/tools/statistics.js";

describe("statistics Zod Schema", () => {
  it("should have a valid Zod object schema", () => {
    expect(statisticsSchema).toBeDefined();
    expect(statisticsSchema instanceof z.ZodObject).toBe(true);
  });

  it("should enforce required fields (if any)", () => {
    // We dynamically check required fields from the parsed schema shape
    const shape = statisticsSchema.shape;
  ...
  it("should reject invalid data types", () => {
    ...
    const resultString = statisticsSchema.safeParse("not-an-object");
    expect(resultString.success).toBe(false);

    const resultNumber = statisticsSchema.safeParse(42);
    expect(resultNumber.success).toBe(false);

    const resultArray = statisticsSchema.safeParse([]);
