/**
 * Tests for MCP Tool: users_update
 *
...
import { z } from "zod";
import { users_updateSchema } from "../src/tools/users_update.js";

describe("users_update Zod Schema", () => {
  it("should have a valid Zod object schema", () => {
    expect(users_updateSchema).toBeDefined();
    expect(users_updateSchema instanceof z.ZodObject).toBe(true);
  });

  it("should enforce required fields (if any)", () => {
    // We dynamically check required fields from the parsed schema shape
    const shape = users_updateSchema.shape;
    ...
    if (requiredKeys.length === 0) {
      ...
      const result = users_updateSchema.safeParse({});
  ...
  it("should reject invalid data types", () => {
    ...
    const resultString = users_updateSchema.safeParse("not-an-object");
    expect(resultString.success).toBe(false);

    const resultNumber = users_updateSchema.safeParse(42);
    expect(resultNumber.success).toBe(false);

    const resultArray = users_updateSchema.safeParse([]);
