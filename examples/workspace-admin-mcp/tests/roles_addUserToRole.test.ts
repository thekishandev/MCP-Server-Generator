/**
 * Tests for MCP Tool: roles_addUserToRole
 *
...
import { z } from "zod";
import { roles_addUserToRoleSchema } from "../src/tools/roles_addUserToRole.js";

describe("roles_addUserToRole Zod Schema", () => {
  it("should have a valid Zod object schema", () => {
    expect(roles_addUserToRoleSchema).toBeDefined();
    expect(roles_addUserToRoleSchema instanceof z.ZodObject).toBe(true);
  });

  it("should enforce required fields (if any)", () => {
    // We dynamically check required fields from the parsed schema shape
    const shape = roles_addUserToRoleSchema.shape;
    ...
    if (requiredKeys.length === 0) {
      ...
      const result = roles_addUserToRoleSchema.safeParse({});
  ...
  it("should reject invalid data types", () => {
    ...
    const resultString = roles_addUserToRoleSchema.safeParse("not-an-object");
    expect(resultString.success).toBe(false);

    const resultNumber = roles_addUserToRoleSchema.safeParse(42);
    expect(resultNumber.success).toBe(false);

    const resultArray = roles_addUserToRoleSchema.safeParse([]);
