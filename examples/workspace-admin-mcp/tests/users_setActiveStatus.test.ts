/**
 * Tests for MCP Tool: users_setActiveStatus
 *
...
import { z } from "zod";
import { users_setActiveStatusSchema } from "../src/tools/users_setActiveStatus.js";

describe("users_setActiveStatus Zod Schema", () => {
  it("should have a valid Zod object schema", () => {
    expect(users_setActiveStatusSchema).toBeDefined();
    expect(users_setActiveStatusSchema instanceof z.ZodObject).toBe(true);
  });

  it("should enforce required fields (if any)", () => {
    // We dynamically check required fields from the parsed schema shape
    const shape = users_setActiveStatusSchema.shape;
    ...
    if (requiredKeys.length === 0) {
      ...
      const result = users_setActiveStatusSchema.safeParse({});
  ...
  it("should reject invalid data types", () => {
    ...
    const resultString = users_setActiveStatusSchema.safeParse("not-an-object");
    expect(resultString.success).toBe(false);

    const resultNumber = users_setActiveStatusSchema.safeParse(42);
    expect(resultNumber.success).toBe(false);

    const resultArray = users_setActiveStatusSchema.safeParse([]);
