...
import { z } from "zod";
import { settingsSchema } from "../src/tools/settings.js";

describe("settings Zod Schema", () => {
  it("should have a valid Zod object schema", () => {
    expect(settingsSchema).toBeDefined();
    expect(settingsSchema instanceof z.ZodObject).toBe(true);
  });
  ...
  it("should reject invalid data types", () => {
    ...
    const resultString = settingsSchema.safeParse("not-an-object");
    expect(resultString.success).toBe(false);

    const resultNumber = settingsSchema.safeParse(42);
    expect(resultNumber.success).toBe(false);

    const resultArray = settingsSchema.safeParse([]);
