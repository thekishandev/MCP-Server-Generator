/**
 * Tests for MCP Tool: roles_update
 *
...
import { z } from "zod";
import { roles_updateSchema } from "../src/tools/roles_update.js";

describe("roles_update Zod Schema", () => {
  it("should have a valid Zod object schema", () => {
    expect(roles_updateSchema).toBeDefined();
    expect(roles_updateSchema instanceof z.ZodObject).toBe(true);
  });

  it("should enforce required fields (if any)", () => {
    // We dynamically check required fields from the parsed schema shape
    const shape = roles_updateSchema.shape;
    ...
    if (requiredKeys.length === 0) {
      ...
      const result = roles_updateSchema.safeParse({});
  ...
  it("should reject invalid data types", () => {
    ...
    const resultString = roles_updateSchema.safeParse("not-an-object");
    expect(resultString.success).toBe(false);

    const resultNumber = roles_updateSchema.safeParse(42);
    expect(resultNumber.success).toBe(false);

    const resultArray = roles_updateSchema.safeParse([]);
