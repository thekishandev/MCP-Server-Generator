/**
 * Tests for MCP Tool: chat_getPinnedMessages
 *
...
import { z } from "zod";
import { chat_getPinnedMessagesSchema } from "../src/tools/chat_getPinnedMessages.js";

describe("chat_getPinnedMessages Zod Schema", () => {
  it("should have a valid Zod object schema", () => {
    expect(chat_getPinnedMessagesSchema).toBeDefined();
    expect(chat_getPinnedMessagesSchema instanceof z.ZodObject).toBe(true);
  });

  it("should enforce required fields (if any)", () => {
    // We dynamically check required fields from the parsed schema shape
    const shape = chat_getPinnedMessagesSchema.shape;
    ...
    if (requiredKeys.length === 0) {
      ...
      const result = chat_getPinnedMessagesSchema.safeParse({});
  ...
  it("should reject invalid data types", () => {
    ...
    const resultString = chat_getPinnedMessagesSchema.safeParse("not-an-object");
    expect(resultString.success).toBe(false);

    const resultNumber = chat_getPinnedMessagesSchema.safeParse(42);
    expect(resultNumber.success).toBe(false);

    const resultArray = chat_getPinnedMessagesSchema.safeParse([]);
