/**
 * Tests for MCP Tool: rooms_createDiscussion
 *
...
import { z } from "zod";
import { rooms_createDiscussionSchema } from "../src/tools/rooms_createDiscussion.js";

describe("rooms_createDiscussion Zod Schema", () => {
  it("should have a valid Zod object schema", () => {
    expect(rooms_createDiscussionSchema).toBeDefined();
    expect(rooms_createDiscussionSchema instanceof z.ZodObject).toBe(true);
  });

  it("should enforce required fields (if any)", () => {
    // We dynamically check required fields from the parsed schema shape
    const shape = rooms_createDiscussionSchema.shape;
    ...
    if (requiredKeys.length === 0) {
      ...
      const result = rooms_createDiscussionSchema.safeParse({});
  ...
  it("should reject invalid data types", () => {
    ...
    const resultString = rooms_createDiscussionSchema.safeParse("not-an-object");
    expect(resultString.success).toBe(false);

    const resultNumber = rooms_createDiscussionSchema.safeParse(42);
    expect(resultNumber.success).toBe(false);

    const resultArray = rooms_createDiscussionSchema.safeParse([]);
