...
import { z } from "zod";
import { users_listSchema } from "../src/tools/users_list.js";

describe("users_list Zod Schema", () => {
  it("should have a valid Zod object schema", () => {
    expect(users_listSchema).toBeDefined();
    expect(users_listSchema instanceof z.ZodObject).toBe(true);
  });

  it("should enforce required fields (if any)", () => {
    // We dynamically check required fields from the parsed schema shape
    const shape = users_listSchema.shape;
    ...
    if (requiredKeys.length === 0) {
      ...
      const result = users_listSchema.safeParse({});
  ...
  it("should reject invalid data types", () => {
    ...
    const resultString = users_listSchema.safeParse("not-an-object");
    expect(resultString.success).toBe(false);

    const resultNumber = users_listSchema.safeParse(42);
    expect(resultNumber.success).toBe(false);

    const resultArray = users_listSchema.safeParse([]);
