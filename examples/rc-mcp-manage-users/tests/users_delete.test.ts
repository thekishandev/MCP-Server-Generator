/**
 * Tests for MCP Tool: users_delete
 *
...
import { z } from "zod";
import { users_deleteSchema } from "../src/tools/users_delete.js";

describe("users_delete Zod Schema", () => {
  it("should have a valid Zod object schema", () => {
    expect(users_deleteSchema).toBeDefined();
    expect(users_deleteSchema instanceof z.ZodObject).toBe(true);
  });

  it("should enforce required fields (if any)", () => {
    // We dynamically check required fields from the parsed schema shape
    const shape = users_deleteSchema.shape;
    ...
    if (requiredKeys.length === 0) {
      ...
      const result = users_deleteSchema.safeParse({});
  ...
  it("should reject invalid data types", () => {
    ...
    const resultString = users_deleteSchema.safeParse("not-an-object");
    expect(resultString.success).toBe(false);

    const resultNumber = users_deleteSchema.safeParse(42);
    expect(resultNumber.success).toBe(false);

    const resultArray = users_deleteSchema.safeParse([]);
