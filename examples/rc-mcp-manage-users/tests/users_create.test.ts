/**
 * Tests for MCP Tool: users_create
 *
...
import { z } from "zod";
import { users_createSchema } from "../src/tools/users_create.js";

describe("users_create Zod Schema", () => {
  it("should have a valid Zod object schema", () => {
    expect(users_createSchema).toBeDefined();
    expect(users_createSchema instanceof z.ZodObject).toBe(true);
  });

  it("should enforce required fields (if any)", () => {
    // We dynamically check required fields from the parsed schema shape
    const shape = users_createSchema.shape;
    ...
    if (requiredKeys.length === 0) {
      ...
      const result = users_createSchema.safeParse({});
  ...
  it("should reject invalid data types", () => {
    ...
    const resultString = users_createSchema.safeParse("not-an-object");
    expect(resultString.success).toBe(false);

    const resultNumber = users_createSchema.safeParse(42);
    expect(resultNumber.success).toBe(false);

    const resultArray = users_createSchema.safeParse([]);
