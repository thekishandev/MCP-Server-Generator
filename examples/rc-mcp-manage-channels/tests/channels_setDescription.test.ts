/**
 * Tests for MCP Tool: channels_setDescription
 *
...
import { z } from "zod";
import { channels_setDescriptionSchema } from "../src/tools/channels_setDescription.js";

describe("channels_setDescription Zod Schema", () => {
  it("should have a valid Zod object schema", () => {
    expect(channels_setDescriptionSchema).toBeDefined();
    expect(channels_setDescriptionSchema instanceof z.ZodObject).toBe(true);
  });

  it("should enforce required fields (if any)", () => {
    // We dynamically check required fields from the parsed schema shape
    const shape = channels_setDescriptionSchema.shape;
    ...
    if (requiredKeys.length === 0) {
      ...
      const result = channels_setDescriptionSchema.safeParse({});
  ...
  it("should reject invalid data types", () => {
    ...
    const resultString = channels_setDescriptionSchema.safeParse("not-an-object");
    expect(resultString.success).toBe(false);

    const resultNumber = channels_setDescriptionSchema.safeParse(42);
    expect(resultNumber.success).toBe(false);

    const resultArray = channels_setDescriptionSchema.safeParse([]);
