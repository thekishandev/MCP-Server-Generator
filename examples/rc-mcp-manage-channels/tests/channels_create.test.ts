/**
 * Tests for MCP Tool: channels_create
 *
...
import { z } from "zod";
import { channels_createSchema } from "../src/tools/channels_create.js";

describe("channels_create Zod Schema", () => {
  it("should have a valid Zod object schema", () => {
    expect(channels_createSchema).toBeDefined();
    expect(channels_createSchema instanceof z.ZodObject).toBe(true);
  });

  it("should enforce required fields (if any)", () => {
    // We dynamically check required fields from the parsed schema shape
    const shape = channels_createSchema.shape;
    ...
    if (requiredKeys.length === 0) {
      ...
      const result = channels_createSchema.safeParse({});
  ...
  it("should reject invalid data types", () => {
    ...
    const resultString = channels_createSchema.safeParse("not-an-object");
    expect(resultString.success).toBe(false);

    const resultNumber = channels_createSchema.safeParse(42);
    expect(resultNumber.success).toBe(false);

    const resultArray = channels_createSchema.safeParse([]);
