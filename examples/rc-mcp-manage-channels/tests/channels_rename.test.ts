/**
 * Tests for MCP Tool: channels_rename
 *
...
import { z } from "zod";
import { channels_renameSchema } from "../src/tools/channels_rename.js";

describe("channels_rename Zod Schema", () => {
  it("should have a valid Zod object schema", () => {
    expect(channels_renameSchema).toBeDefined();
    expect(channels_renameSchema instanceof z.ZodObject).toBe(true);
  });

  it("should enforce required fields (if any)", () => {
    // We dynamically check required fields from the parsed schema shape
    const shape = channels_renameSchema.shape;
    ...
    if (requiredKeys.length === 0) {
      ...
      const result = channels_renameSchema.safeParse({});
  ...
  it("should reject invalid data types", () => {
    ...
    const resultString = channels_renameSchema.safeParse("not-an-object");
    expect(resultString.success).toBe(false);

    const resultNumber = channels_renameSchema.safeParse(42);
    expect(resultNumber.success).toBe(false);

    const resultArray = channels_renameSchema.safeParse([]);
