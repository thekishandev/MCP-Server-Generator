/**
 * Tests for MCP Tool: send_dm_to_user
 *
...
import { z } from "zod";
import { send_dm_to_userSchema, register_send_dm_to_user } from "../src/tools/send_dm_to_user.js";

describe("send_dm_to_user Zod Schema", () => {
  it("should have a valid Zod object schema", () => {
    expect(send_dm_to_userSchema).toBeDefined();
    expect(send_dm_to_userSchema instanceof z.ZodObject).toBe(true);
  });

  it("should enforce required fields (if any)", () => {
    // We dynamically check required fields from the parsed schema shape
    const shape = send_dm_to_userSchema.shape;
    ...
    if (requiredKeys.length === 0) {
      ...
      const result = send_dm_to_userSchema.safeParse({});
  ...
  it("should reject invalid data types", () => {
    ...
    const resultString = send_dm_to_userSchema.safeParse("not-an-object");
    expect(resultString.success).toBe(false);

    const resultNumber = send_dm_to_userSchema.safeParse(42);
    expect(resultNumber.success).toBe(false);

    const resultArray = send_dm_to_userSchema.safeParse([]);
    expect(resultArray.success).toBe(false);
  });

  it("should export a register function for MCP server integration", () => {
    expect(register_send_dm_to_user).toBeDefined();
    expect(typeof register_send_dm_to_user).toBe("function");
  });
});
