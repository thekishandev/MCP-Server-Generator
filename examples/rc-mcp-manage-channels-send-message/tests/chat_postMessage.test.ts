/**
 * Tests for MCP Tool: chat_postMessage
 *
...
import { z } from "zod";
import { chat_postMessageSchema } from "../src/tools/chat_postMessage.js";

describe("chat_postMessage Zod Schema", () => {
  it("should have a valid Zod object schema", () => {
    expect(chat_postMessageSchema).toBeDefined();
    expect(chat_postMessageSchema instanceof z.ZodObject).toBe(true);
  });

  it("should enforce required fields (if any)", () => {
    // We dynamically check required fields from the parsed schema shape
    const shape = chat_postMessageSchema.shape;
    ...
    if (requiredKeys.length === 0) {
      ...
      const result = chat_postMessageSchema.safeParse({});
  ...
  it("should reject invalid data types", () => {
    ...
    const resultString = chat_postMessageSchema.safeParse("not-an-object");
    expect(resultString.success).toBe(false);

    const resultNumber = chat_postMessageSchema.safeParse(42);
    expect(resultNumber.success).toBe(false);

    const resultArray = chat_postMessageSchema.safeParse([]);
