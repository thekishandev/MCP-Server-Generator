/**
 * Tests for MCP Tool: channels_list
 *
...
import { z } from "zod";
import { channels_listSchema } from "../src/tools/channels_list.js";

describe("channels_list Zod Schema", () => {
  it("should have a valid Zod object schema", () => {
    expect(channels_listSchema).toBeDefined();
    expect(channels_listSchema instanceof z.ZodObject).toBe(true);
  });

  it("should enforce required fields (if any)", () => {
    // We dynamically check required fields from the parsed schema shape
    const shape = channels_listSchema.shape;
    ...
    if (requiredKeys.length === 0) {
      ...
      const result = channels_listSchema.safeParse({});
  ...
  it("should reject invalid data types", () => {
    ...
    const resultString = channels_listSchema.safeParse("not-an-object");
    expect(resultString.success).toBe(false);

    const resultNumber = channels_listSchema.safeParse(42);
    expect(resultNumber.success).toBe(false);

    const resultArray = channels_listSchema.safeParse([]);
