/**
 * Tests for MCP Tool: channels_history
 *
...
import { z } from "zod";
import { channels_historySchema } from "../src/tools/channels_history.js";

describe("channels_history Zod Schema", () => {
  it("should have a valid Zod object schema", () => {
    expect(channels_historySchema).toBeDefined();
    expect(channels_historySchema instanceof z.ZodObject).toBe(true);
  });

  it("should enforce required fields (if any)", () => {
    // We dynamically check required fields from the parsed schema shape
    const shape = channels_historySchema.shape;
    ...
    if (requiredKeys.length === 0) {
      ...
      const result = channels_historySchema.safeParse({});
  ...
  it("should reject invalid data types", () => {
    ...
    const resultString = channels_historySchema.safeParse("not-an-object");
    expect(resultString.success).toBe(false);

    const resultNumber = channels_historySchema.safeParse(42);
    expect(resultNumber.success).toBe(false);

    const resultArray = channels_historySchema.safeParse([]);
