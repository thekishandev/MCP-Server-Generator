/**
 * Tests for MCP Tool: chat_search
 *
...
import { z } from "zod";
import { chat_searchSchema } from "../src/tools/chat_search.js";

describe("chat_search Zod Schema", () => {
  it("should have a valid Zod object schema", () => {
    expect(chat_searchSchema).toBeDefined();
    expect(chat_searchSchema instanceof z.ZodObject).toBe(true);
  });

  it("should enforce required fields (if any)", () => {
    // We dynamically check required fields from the parsed schema shape
    const shape = chat_searchSchema.shape;
    ...
    if (requiredKeys.length === 0) {
      ...
      const result = chat_searchSchema.safeParse({});
  ...
  it("should reject invalid data types", () => {
    ...
    const resultString = chat_searchSchema.safeParse("not-an-object");
    expect(resultString.success).toBe(false);

    const resultNumber = chat_searchSchema.safeParse(42);
    expect(resultNumber.success).toBe(false);

    const resultArray = chat_searchSchema.safeParse([]);
